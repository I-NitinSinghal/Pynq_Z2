...
#pragma HLS PIPELINE II=1
        ...
        if (x < HALF_WIDTH) {
            ...
            cost_t best_cost   = 255;
            cost_t second_best = 255;
...
#pragma HLS UNROLL
            ...
            bool in_valid_zone =
                (xr >= (ap_uint<11>)MAX_DISP) &&
                (xr <  (ap_uint<11>)(HALF_WIDTH - AGG_HALF_W)) &&
                (y  >= 1);


            // Uniqueness check (25% margin)
            int margin = (int)best_cost + ((int)best_cost >> 2);

            bool unique =
                ((int)second_best > margin);
            ...
            if (in_valid_zone && unique && confident)
            {
